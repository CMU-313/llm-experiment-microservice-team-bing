...
def get_translation(post: str, lang: str) -> str:
    response = client.chat(
        ...
        messages=[
            {
                "role": "user",
                "content": f"Translate this {lang if lang else 'given'} text to English. Respond ONLY with the English translation. If it's gibberish, return just the post: {post}"
            }
        ],
